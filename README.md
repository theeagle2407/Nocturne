...
- **Paper-trading log (highest-tier evidence):** [`paper_trading_log.csv`](./paper_trading_log.csv) — the Exposure Planner's nightly HOLD / TRIM / FLATTEN decisions replayed **causally** on real data: **690 nights, 4,596 overnight trades** (HOLD 4,391 · TRIM 205 · FLATTEN 234), ending **−16.6% net of a 0.10% per-night cost**. The risk overlay trims into volatility and flattens on FOMC nights. The net loss is the point — it confirms the overnight premium doesn't survive realistic costs, so exposure must be *managed*, not traded. Generated by [`paper_trading_log.js`](./paper_trading_log.js) (rule in code, not a screenshot); methodology in [`PAPER_TRADING.md`](./PAPER_TRADING.md).
